...
import { Ticket, Calendar, Clock, ExternalLink, TrendingUp } from "lucide-react";
...
const PublicRaffles = () => {
  ...
  const fetchRaffles = async () => {
    try {
      ...
      // Fetch ticket stats for each raffle
      const rafflesWithStats = await Promise.all(
        (data || []).map(async (raffle) => {
          ...
          const sold = tickets?.filter(t => t.estado !== "disponible" && t.estado !== "libre").length || 0;
          return { ...raffle, soldCount: sold, totalCount: total };
        })
      );
      ...
      <main className="flex-1 container mx-auto px-4 py-12">
        ...
        {raffles.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-7xl mx-auto">
            {raffles.map((raffle) => {
              const progress = raffle.totalCount ? ((raffle.soldCount || 0) / raffle.totalCount * 100) : 0;
              return (
                <Card
                  key={raffle.id}
                  className="border-primary/20 shadow-glow hover:shadow-card transition-all duration-300 hover:-translate-y-1 cursor-pointer overflow-hidden group"
        ...
        <div className="container mx-auto px-4 py-6">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="text-center md:text-left">
              ...
              <p className="text-xs text-muted-foreground mt-1">Soluciones tecnológicas profesionales</p>
            </div>
            ...
            <Button 
              ...
              variant="outline" 
              size="sm"
              className="border-success/50 text-success hover:bg-success/10 transition-colors"
            >

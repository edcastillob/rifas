...
export const AdminManagement = () => {
  ...
  const [userToChangePassword, setUserToChangePassword] = useState<Admin | null>(null);
  ...
  const fetchAdmins = async () => {
    ...
    const adminsWithEmails = await Promise.all(
      (roles || []).map(async (role) => {
        const { data: email } = await supabase.rpc("get_user_email", { _user_id: role.user_id });
  ...
  const handleCreateAdmin = async (e: React.FormEvent) => {
    ...
    if (authData.user) {
      const { error: roleError } = await supabase
        .from("user_roles")
        .insert({ user_id: authData.user.id, role: "admin", must_change_password: true });

      if (roleError) {
        toast({ title: "Error", description: "No se pudo asignar el rol de administrador", variant: "destructive" });
      } else {
        toast({ title: "Éxito", description: "Administrador creado. Deberá cambiar su contraseña en el primer inicio." });
        setNewEmail("");
        setNewPassword("");
        fetchAdmins();
      }
  ...
  const handleDeleteAdmin = async (userId: string, email: string) => {
    if (email === "edwar.castillo@gmail.com") {
      toast({ title: "Error", description: "No se puede eliminar al super administrador", variant: "destructive" });
      return;
    }
    ...
    if (error) {
      ...
      toast({ title: "Éxito", description: "Administrador eliminado correctamente" });
      fetchAdmins();
    }
  };

  const handleChangePassword = async () => {
    if (!userToChangePassword || !newUserPassword) return;
    ...
    try {
      const { data, error } = await supabase.functions.invoke("admin-change-password", {
        body: { userId: userToChangePassword.user_id, newPassword: newUserPassword },
      });

      if (error) throw error;
      if (data?.error) throw new Error(data.error);

      toast({ title: "Éxito", description: `Contraseña cambiada para ${userToChangePassword.email}` });
    } catch (err: any) {
    ...
    <Card className="border-primary/20 shadow-card">
      ...
      <CardContent className="space-y-6">
        <form onSubmit={handleCreateAdmin} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input id="admin-email" type="email" placeholder="admin@ejemplo.com" value={newEmail} onChange={(e) => setNewEmail(e.target.value)} required />
            </div>
            <div className="space-y-2">
              <Label htmlFor="admin-password">Contraseña</Label>
              <Input id="admin-password" type="password" placeholder="••••••••" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} required />
            </div>
          </div>
        ...
        <div className="space-y-2">
          ...
          <div className="space-y-2">
            {admins.map((admin) => (
              <div key={admin.id} className="flex items-center justify-between p-3 border rounded-lg border-border">
                <div>
                ...
                <div className="flex items-center gap-2">
                  {admin.email !== "edwar.castillo@gmail.com" && (
                    <>
                      <Button variant="outline" size="sm" onClick={() => { setUserToChangePassword(admin); setNewUserPassword(""); }}>
                        <KeyRound className="h-4 w-4" />
                      </Button>
                      <Button variant="destructive" size="sm" onClick={() => handleDeleteAdmin(admin.user_id, admin.email)}>
      ...
      <AlertDialog open={!!userToChangePassword} onOpenChange={() => { setUserToChangePassword(null); setNewUserPassword(""); }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Cambiar Contraseña</AlertDialogTitle>
            <AlertDialogDescription>
              Cambiar la contraseña de <strong>{userToChangePassword?.email}</strong>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <div className="py-4">
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={handleChangePassword} disabled={newUserPassword.length < 6}>
              Cambiar Contraseña
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
